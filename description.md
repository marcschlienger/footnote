...
- **done / failed** — terminal. Push notifications fire for both.
...
- **Source copies** are written first, so the report can link them:
  ...
  matches the citation numbering for the sources that have copies.
...
`GET /jobs/{id}/report` strips the frontmatter and renders the Markdown
...
- **Configuration surface**: on load the app calls `/health` and shows a
  red flash if `PARALLEL_API_KEY` is missing, and the footer states where
  dossiers are being saved — the two things you most want to know on a
  fresh install.
- **Service worker**: network-first with cache fallback for the shell
  (`/`, CSS, JS, manifest, icons) so updates arrive immediately but the app
  opens offline; API requests are never cached — stale research status is
  worse than an error. Push and notification-click handlers complete the
  notification loop.
- **Offline**: the shell opens and shows the last-known nothing; submitting
  fails visibly. Queued offline capture (à la background sync) is
  deliberately out of scope — a research request needs the server anyway.
...
| `static/service-worker.js` | offline shell cache + push/notification handlers |
...
**macOS — Launch Agent (auto-starts at login):** save as
`~/Library/LaunchAgents/com.marc.footnote.plist`, adjusting the two paths:
...
<plist version="1.0"><dict>
  <key>Label</key><string>com.marc.footnote</string>
  <key>ProgramArguments</key>
    <array><string>/Users/marc/Projects/footnote/start.sh</string></array>
  <key>RunAtLoad</key><true/>
  <key>KeepAlive</key><true/>
  <key>StandardOutPath</key>
    <string>/Users/marc/Projects/footnote/server.log</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.marc.footnote.plist
